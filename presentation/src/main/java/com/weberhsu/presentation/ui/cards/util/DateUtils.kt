...
internal object DateUtils {
    ...
    private const val THRESS_DIGITAL = 3
    private const val MAX_MONTH = 12
    private const val MAX_VALID_YEAR = 9980
    private const val INTENTIONAL_INTEGER_DIVISION = 100
    private const val EIGHTY = 80
    ...
    fun isValidMonth(monthString: String?): Boolean {
        return runCatching {
            monthString?.toInt() in 1..MAX_MONTH
        }.getOrDefault(false)
    }
    ...
    fun isExpiryDataValid(expiryMonth: Int, expiryYear: Int, calendar: Calendar): Boolean {
        if (expiryMonth < 1 || expiryMonth > MAX_MONTH) {
            return false
        }
    ...
    fun createDateStringFromIntegerInput(
        ...
        if (yearString.length == THRESS_DIGITAL) {
            return ""
        }
    ...
    fun convertTwoDigitYearToFour(
        ...
        var centuryBase = year / INTENTIONAL_INTEGER_DIVISION
        if (year % INTENTIONAL_INTEGER_DIVISION > EIGHTY && inputYear < TWENTY) {
            centuryBase++
        } else if (year % INTENTIONAL_INTEGER_DIVISION < TWENTY && inputYear > EIGHTY) {
            centuryBase--
        }
        return centuryBase * INTENTIONAL_INTEGER_DIVISION + inputYear
    }
